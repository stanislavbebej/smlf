<?
function DoQuine_silent($function_name)
{
...
//        print("<br>(".$iteracia.")<br>\n");
        $again = Quine_silent($to_do_array_name, $completed_name); //$again je navratova hodnota, vdaka nej je mozne vediet, ci boli vytvorene nove neurcite vektory spojenim dvoch predchadzajucich prvkov. Ak ano, pole $to_do_array_name nie je urcite prazdne, a teda ho treba opat spracovat
        $iteracia++;
    } while ($again == 1); //(sizeof($$to_do_array_name) != 0);    //dokial vznikli nove neuplne vektory, alebodokial je co spracovavat v poli $$to_do_array_name
